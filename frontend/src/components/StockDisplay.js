...
const StockDisplay = (props) => {
  ...
  let quantityClass =
    "rounded-full w-fit text-sm px-2 bg-[#FCEBEC] text-[#9E2930]"; // Default red for <5

  if (itemQuantity > 10) {
    quantityClass =
      "rounded-full w-fit text-sm px-2 bg-[#E6F4EA] text-[#2E7D32]"; // Green for >10
  } else if (itemQuantity >= 5 && itemQuantity <= 10) {
    quantityClass =
      "rounded-full w-fit text-sm px-2 bg-[#FFF4E5] text-[#F57C00]"; // Yellow for 5-10
  }
  return (
    <li key={itemId}>
